...
Xiao Fei (china-fixing@hotmail.com, xiao.fei@sund.ku.dk)
    ...
    parser.add_argument('--OUTPUT', default="core_snp", type=str, metavar='FILENAME', help="Output bed-file name, default is core_snp")
...
def read_matrix(input):
    snp_matrix = pd.read_csv(input, sep='\t', header=0, low_memory=False).iloc[:,:-1]
    # print(snp_matrix)
...
### core SNP analysis ###
def analysis(data, cutoff, outname):
    ...
    for sites in data.columns[:-2]:
        ...
        if result == 0:
            ...
            print(ref_name+'\t'+tsites+'\t'+ tsites + '\t' + 'Conserved_SNP', file=open(outname+".bed", "a"))
        else:
            print(ref_name+'\t'+tsites+'\t'+ tsites + '\t' + 'not_SNP_Conserved', file=open(outname+".bed", "a"))
...
def main():
    ...
    args=parse_args()
    print(args)
    original = args.OUTPUT+".bed"
    output = args.OUTPUT+"_old.bed"
    try:
        os.rename(original, output)
    except FileNotFoundError:
        pass

    snp_matrix_t = read_matrix(args.IN_MATRIX)
    data = add_set(args.IN_GROUP, snp_matrix_t)
    analysis(data, args.CUT_OFF, args.OUTPUT)
